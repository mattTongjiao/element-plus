...
describe('Color-picker', () => {
  it('Mount Color-picker', () => {
    ...
    expect(wrapper.html()).toContain('tj-color-picker')
    wrapper.unmount()
  })
  it('should show alpha slider when show-alpha=true', async () => {
    const wrapper = _mount(
      `<tj-color-picker v-model="color" :show-alpha="true"></tj-color-picker>`,
      () => ({
        color: '#20A0FF',
      }),
    )
    await wrapper.find('.tj-color-picker__trigger').trigger('click')
    const alphaSlider = document.querySelector('.tj-color-alpha-slider')
  ...
  it('should show correct rgb value', async () => {
    ...
    await wrapper.find('.tj-color-picker__trigger').trigger('click')
    await nextTick()
    const input = document.querySelector<HTMLInputElement>('.tj-color-dropdown__value input')
  ...
  it('should pick a color when confirm button click', async () => {
    ...
    await wrapper.find('.tj-color-picker__trigger').trigger('click')
    document.querySelector<HTMLElement>('.tj-color-dropdown__btn').click()
    await nextTick()
  ...
  it('should init the right color when open', async () => {
    ...
    const thumbDom = hueSlideWrapper.find<HTMLElement>('.tj-color-hue-slider__thumb').element
  ...
  it('should show color picker when click trigger', async () => {
    ...
    await wrapper.find('.tj-color-picker__trigger').trigger('click')
    const dropdown = document.querySelector('.tj-color-dropdown')
  ...
  it('should clear a color when clear button click', async () => {
    ...
    await  wrapper.find('.tj-color-picker__trigger').trigger('click')
    const clearBtn = document.querySelector<HTMLElement>('.tj-color-dropdown__link-btn')
    clearBtn.click()
  ...
  it('should change alpha when clicking the alpha bar', async () => {
    ...
    const thumbDom = alphaWrapper.find<HTMLElement>('.tj-color-alpha-slider__thumb').element
  ...
  it('should change saturation and value when clicking the sv-panel', async () => {
    const wrapper = _mount(
      `<tj-color-picker v-model="color" show-alpha color-format="hsv"></tj-color-picker>`,
      () => ({
        color: 'hsv(0, 50%, 50%)',
      }),
    )
  ...
  it('should change color to the selected color', async () => {
    const wrapper = _mount(
      `<tj-color-picker v-model="color" show-alpha :predefine="colors"></tj-color-picker>`,
      () => ({
    ...
    expect(predefineDom.querySelectorAll('.tj-color-predefine__color-selector').length === 9).toBeTruthy()
    predefineDom.querySelector<HTMLElement>('.tj-color-predefine__color-selector:nth-child(4)').click()
    await nextTick()
    ...
    predefineDom.querySelector<HTMLElement>('.tj-color-predefine__color-selector:nth-child(3)').click()
    await nextTick()
  ...
  it('should change selected state of predefined color', async () => {
    ...
    predefineDom.querySelector<HTMLElement>('.tj-color-predefine__color-selector:nth-child(4)').click()
    await nextTick()
    expect(predefineWrapper.find('.tj-color-predefine__color-selector:nth-child(4)').classes()).toContain('selected')
    const hueSlideWrapper = colorPickerWrapper.findComponent({ ref: 'hue' })
    const hueSlideDom = hueSlideWrapper.element
    const thumbDom = hueSlideWrapper.find<HTMLElement>('.tj-color-hue-slider__thumb').element
    const mockHueSlideRect = jest.spyOn(hueSlideDom,'getBoundingClientRect').mockReturnValue({
    ...
    await nextTick()
    expect(predefineWrapper.find('.tj-color-predefine__color-selector:nth-child(4)').classes()).not.toContain('selected')
    mockHueSlideRect.mockRestore()
    mockThumbDom.mockRestore()
    mockHueSlideOffsetHeight.mockRestore()
  })
