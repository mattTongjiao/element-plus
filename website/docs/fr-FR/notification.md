...
:::demo Tongjiao UI fournit quatre types de notifications: `success`, `warning`, `info` et `error`. Il sont choisis grâce au champs `type`, et n'importe quelle autre valeur sera ignorée. Il existe des méthodes enregistrées pour chaque type, comme dans `open3` et `open4`, qui ne nécessitent donc pas le champs `type`.
...
Tongjiao UI ajoute la méthode `$notify` à `app.config.globalProperties`. Vous pouvez donc appeler `Notification` dans l'instance de Vue comme nous avons fait dans cette page.
...
Dans ce cas vous devrez appeler `TjNotification(options)`. Il existe aussi des méthodes pour chaque type, e.g. `TjNotification.success(options)`. Vous pouvez appeler `TjNotification.closeAll()` pour fermer manuellement toutes les instances.
