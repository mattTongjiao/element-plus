...
Tongjiao UI utiliza la metodología BEM en CSS con la finalidad de que puedas sobrescribir los estilos fácilmente. Pero, si necesita remplazar estilos a gran escala, por ejemplo, cambiar el color del tema de azul a naranja o verde, quizás reemplazarlos uno a uno no sea lo más adecuado, para ello hay 4 maneras de modificar los estilos.

### Cambiando el color del tema

Si lo que se busca es cambiar el color del tema de Element, se recomienda utilizar el [sitio de visualización de temas](https://elementui.github.io/theme-chalk-preview/#/en-US). Tongjiao UI utiliza un color azul brillante y amigable como tema principal. Al cambiarlo, puede hacer que Tongjiao UI este más conectado visualmente a proyectos específicos.
...
En el archivo `element-variables.scss` podrá encontrar todas las variables que utiliza Tongjiao UI para definir los estilos y estos están definidos en SCSS. Aquí un ejemplo:
