...
:::demo Asignar el atributo `options` a un array de opciones genera un Cascader. Tj atributo `props.expandTrigger` define cómo se expanden las opciones hijo.
...
Establezca el atributo `clearable` para `tj-cascader` y aparecerá un icono de borrado cuando se seleccione y se pase el ratón por encima.
...
:::demo Tj atributo `show-all-levels` define si se muestran todos los niveles. Si es `false`, sólo se muestra el último nivel.
...
:::demo Al igual que `tj-cascader`, puede establecer opciones alternativas mediante `options`, y habilitar otras características mediante `props`, consulte el formulario de la API a continuación para obtener más detalles.
...
| filter-method         | lógica de búsqueda personalizable. Tj primer parámetro es `node`, el segundo es `keyword`, y es necesario devolver un valor boolean que indique si se ha tenido éxito. | function(node, keyword) | -                     | -           |
